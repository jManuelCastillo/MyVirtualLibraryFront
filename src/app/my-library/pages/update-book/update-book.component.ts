...
export class UpdateBookComponent {
    ...
    constructor(private formBuilder : FormBuilder, private titlecasePipe : TitleCasePipe, private route : ActivatedRoute, private messageService : MessageService, private storage : Storage, private libraryService : LibraryService, private confirmationService : ConfirmationService,) {
        this.genresInput = [];
        this.uploadedFiles = []
    }
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe(params => {
            this.libraryService.getBookByID(params['id']).then(book => {
    ...
    bookForm : FormGroup = this.formBuilder.group({
        ...
        descriptionInput: [
            ,
            [
                Validators.minLength(0), Validators.maxLength(1500)
            ]
        ],
        physBooknput: ['false']
    })
    ...
    validField(field : string) {
        return this.bookForm.controls[field].errors && this.bookForm.controls[field].touched
    }
    ...
    async saveBook() {
        ...
        if (tempbook !== undefined) {
            if (tempbook.id != this.currentBook.id) {
        ...
        let updatedBook: Book = {
            ...
            isNotAvailableReason: (this.bookForm.value.physBooknput == 'true' && this.bookForm.value.isNotAvailableReason) ? this.bookForm.value.isNotAvailableReason : {
                name: '',
                id: ''
            }
        ...
        this.uploadedFiles.map(file => {
            const filesRef = ref(this.storage, `BooksFiles/${
                file.name
            }`)

            uploadBytes(filesRef, file).then(response => {
                console.log(response)
            }).catch(error => console.log(error));

            updatedBook.files.push({
                    format: file.name.split('.').slice(1), route: `BooksFiles/${
                    file.name
                }`
            });
        })
        ...
        await this.libraryService.deleteFileBook(this.filesToDelete).then(a => {
            this.libraryService.updateBook(updatedBook).then(response => {
